...
class ComponentScanAnnotationParser {
	...
	/**
	 * 1、定义了一个扫描器scanner，还记不记在new AnnotationConfigApplicationContext的时候，
	 * 会调用AnnotationConfigApplicationContext的构造方法，构造方法里面有一句
	 * this.scanner = new ClassPathBeanDefinitionScanner(this);当时说这个对象不重要，这里就是证明了。
	...
	public Set<BeanDefinitionHolder> parse(AnnotationAttributes componentScan, final String declaringClass) {
		/**
		 * 扫描器，还记不记在new AnnotationConfigApplicationContext的时候,会调用AnnotationConfigApplicationContext的构造方法
		 ...
		 *
		 *  1.指定basePackageClasses，就是指定多个类，只要是与这几个类同级的，或者在这几个类下级的都可以被扫描到，
		 *    这种方式其实是spring比较推荐的,因为指定basePackages没有IDE的检查，容易出错，但是指定一个类，就有IDE的检查了，
		 *    不容易出错，经常会用一个空的类来作为basePackageClasses
		 *
